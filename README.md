...
Jamz Player 是一個原生 iPhone MP3 播放器，使用 SwiftUI、AVFoundation 與 MediaPlayer 開發，最低支援 iOS 17。介面採繁體中文、深色背景與暖橘色重點色；不需要帳號，也沒有第三方執行期套件。

## 功能

- 從 HTTPS MP3 直連網址下載音檔，顯示進度並可取消；拒絕 HTTP 與降級至 HTTP 的重新導向。
...
程式測試與模擬器建置不能取代下列實機驗收：

- 鎖定畫面播放、控制中心控制、耳機拔除及來電／音訊中斷。
- 實際 15 分鐘睡眠定時器到期停止，並確認循環不會重新啟動。
- 明確斷網後的離線播放、低儲存空間及傳輸途中斷線。
- 小尺寸 iPhone、放大文字與 VoiceOver 的完整操作。

建議至少以兩首有效 MP3（包含一首有中文 ID3 與封面、另一首無標籤）執行匯入、重開 App、離線播放、編輯、刪除、循環及睡眠定時器流程。
